...
unsigned char key;
int s_flag;
static int index = 0,flag = 0, count = 0, event_count, menu_flag = 0;
static unsigned int eeprom_address = 0X00;
unsigned long int speed;
static int menu_cnt, i = 0, j = 0;
static int start_index = 0, current_index = 0;
static unsigned char time_v[10][15];
unsigned char clock_reg[3];
static unsigned char feild = 0;
...
void display_menu(void) {
    ...
    clcd_print(menu[menu_cnt], (0x80 + (2)));
    clcd_print(menu[menu_cnt + 1], (0xC0 + (2)));
...
void log_event(void) {
    for (int i = 0; i < 8; i++) {
        if (i == 2 || i == 5) {
            ...
            write_external_eeprom(eeprom_address++, time[i]);
        }
    }

    write_external_eeprom(eeprom_address++, arr[count][0]);
    write_external_eeprom(eeprom_address++, arr[count][1]);
...
void view_log(void) {
    ...
    static unsigned int view_address = 0x00;

    static int view_flag = 0;

    if (event_count == 0) {
        clcd_print("NO LOGS..          ", (0x80 + (0)));
        clcd_print("  TO DISPLAY :(    ", (0xC0 + (0)));
    ...
    for (int i = 0; i < event_count; i++) {
        int entry_index = (start_index + i) % 10;
        view_address = entry_index * 10;
    ...
    if (key == 2 && index < event_count - 1) {
        index++;
    } else if (key == 1 && index > 0) {
        index--;
    }

    clcd_print(time_v[index], (0xC0 + (2)));
    clcd_putch(index + 48,(0xC0 + (0)));
}
...
void clear_logs(void) {
    ...
    for (int addr = 0; addr < eeprom_address; addr++) {
        write_external_eeprom(addr, 0xFF);
    }
    eeprom_address = 0;
    event_count = 0;
    menu_cnt = 0;
...
void set_time(void) {
    ...
    if(feild == 0) {
        if (delay++ < 500){
        clcd_putch((hrs / 10)+'0', (0xC0 + (0)));
        clcd_putch((hrs % 10)+'0', (0xC0 + (1)));
        clcd_putch(':', (0xC0 + (2)));
        clcd_putch((mins / 10)+'0', (0xC0 + (3)));
        clcd_putch((mins % 10)+'0', (0xC0 + (4)));
        clcd_putch(':', (0xC0 + (5)));
        clcd_putch((secs / 10)+'0', (0xC0 + (6)));
        clcd_putch((secs % 10)+'0', (0xC0 + (7)));
        }
        else if(delay++ < 1000) {
            clcd_putch(' ', (0xC0 + (0)));
            clcd_putch(' ', (0xC0 + (1)));
    ...
    if(feild == 2) {
        ...
        if (key == 11) {
        s_flag = 0;
        write_ds1307(0x02, ((hrs / 10) << 4) | (hrs % 10));
        write_ds1307(0x01, ((mins / 10) << 4) | (mins % 10));
        write_ds1307(0x00, ((secs / 10) << 4) | (secs % 10));
        get_time();
        flag = 0;
    }
    menu_cnt = 0;
        ...
        if (flag == 0) {
            ...
            if (key == 11) {
                if (menu_cnt == 0 && i == 0) {
                    ...
                    clcd_print("Just wait a sec..", (0xC0 + (0)));
                    _delay((unsigned long)((1000)*(20000000/4000.0)));
                    flag = 3;
                    menu_flag = 2;
                }

                else if ((menu_cnt == 1 && i == 1) || (menu_cnt == 2 && i == 0)) {
                    clcd_write(0x01, 0);
                    flag = 4;
                    menu_flag = 3;
                }
